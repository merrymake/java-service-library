...
public class MerrymakeTests {
    ...
    public void first_action() {
        ...
        Merrymake.service(new String[] { "a", "{ \"messageId\": \"mid\", \"traceId\": \"tid\" }" })
                .handle("a", (payload, envelope) -> {
                    spy[0] = Result.A;
    ...
    public void second_action() {
        final var spy = new Result[] { Result.Nothing };
        Merrymake.service(new String[] { "b", "{ \"messageId\": \"mid\", \"traceId\": \"tid\" }" })
                .handle("a", (payload, envelope) -> {
                    spy[0] = Result.A;
                }).handle("b", (payload, envelope) -> {
                    spy[0] = Result.B;
    ...
    public void init() {
        final var spy = new Result[] { Result.Nothing };
        Merrymake.service(new String[] { "", "{ \"messageId\": \"mid\", \"traceId\": \"tid\" }" })
                .handle("a", (payload, envelope) -> {
                    spy[0] = Result.A;
                }).handle("b", (payload, envelope) -> {
                    spy[0] = Result.B;
